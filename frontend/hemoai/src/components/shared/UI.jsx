...
export function BarChart({ values, labels, color, height = 150 }) {
  const resolvedColor = color || C.red700;
        ...
        return (
          <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
            <div style={{
              ...
              background: isLast ? resolvedColor : `${resolvedColor}`,
...
export function LineChart({ values, labels, color, height = 160 }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H + 8}`} style={{ width: "100%", display: "block" }}>
      ...
      <polyline points={polyline} fill="none" stroke={resolvedColor} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round"/>
      {pts.map((p, i) => (
        <g key={i}>
          <circle cx={p.x} cy={p.y} r={4} fill={C.white} stroke={resolvedColor} strokeWidth={1.8}/>
        </g>
      ))}
