...
def _fetch_donations():
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT BLOOD_GROUP, DONATION_DATE FROM donations"
            )
...
def predict_next_week():
    ...
    # weekly_counts[blood_group][week_number] =
    # how many donations happened that week
    ...
    for group in BLOOD_GROUPS:
        ...
        if weeks_of_data == 0:
            # No history for this blood group yet.
            predictions[group] = {
                "predictedUnits": 0,
                "weeksOfData": 0,
                "method": "no_data",
            ...
            # Carry the only available week's count forward.
            only_count = next(iter(weeks_data.values()))

            predictions[group] = {
                "predictedUnits": only_count,
                "weeksOfData": 1,
                "method": "single_week_carry_forward",
            }

        else:
            # Train a linear regression model using week number
            # as X and donation count as y.
            ...
            # Donation counts cannot be negative.
            predicted = max(0, round(predicted))

            predictions[group] = {
                "predictedUnits": int(predicted),
                "weeksOfData": weeks_of_data,
